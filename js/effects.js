...
Game.Effects = (function () {
  ...
  class Effects {
    ...
    burstFlash(pos, color, size, dur, peak) {
      const mat = new THREE.SpriteMaterial({ map: GLOW_TEX, color, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false });
    ...
    shockRing(pos, normal, color, max, dur, width, opacity) {
      const mat = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: opacity === undefined ? 1 : opacity, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, toneMapped: false });
    ...
    goalExplosion(pos, teamHex, type) {
      ...
      (byType[type] || this.goalClassic).call(this, pos.clone(), team, n);
    }

    goalClassic(pos, team, n) {
      const hot = team.clone().lerp(WHITE, 0.55);
      this.burstFlash(pos, WHITE, 5200, 0.45, 1);
      ...
      for (let i = 0; i < 900; i++) {
        ...
        const c = Math.random() < 0.2 ? tmpC.copy(WHITE) : tmpC.copy(team).offsetHSL(rand(-0.03, 0.03), 0, rand(-0.05, 0.2));
        this.glow.emit(pos.x, pos.y, pos.z, d.x, d.y, d.z, rand(0.9, 2.3), rand(70, 200), c, { drag: 1.5, gravity: 850, grow: -0.5, opacity: 0.9 });
      }
      for (let i = 0; i < 160; i++) {
    ...
    goalPartyTime(pos, team, n) {
      ...
      [-1, 1].forEach(s => {
        ...
        for (let i = 0; i < 420; i++) {
          ...
          this.confetti.emit(origin, dir, PARTY[i % PARTY.length], rand(22, 36), rand(3, 5.5), { gravity: 260, drag: 1.4, spin: 9, flutter: 180 });
        }
        for (let i = 0; i < 26; i++) {
          const dir = n.clone().add(new THREE.Vector3(-s * rand(0.1, 0.5), rand(0.8, 1.5), 0)).normalize().multiplyScalar(rand(900, 1600));
          this.confetti.emit(origin, dir, PARTY[i % PARTY.length], rand(9, 12), rand(3.5, 5), { gravity: 200, drag: 1.6, spin: 4, flutter: 120, stretch: 7 });
        }
      });
      for (let i = 0; i < 360; i++) {
        ...
        this.confetti.emit(pos, d, PARTY[i % PARTY.length], rand(20, 32), rand(2.5, 4.5), { gravity: 260, drag: 1.5, spin: 10, flutter: 160 });
      }
      for (let i = 0; i < 16; i++) {
        const start = new THREE.Vector3(rand(-720, 720), rand(60, 320), pos.z + n.z * rand(60, 700));
        this.balloon(start, PARTY[i % 6].clone(), rand(0, 0.9));
      }
      this.addShake(0.8);
    }

    balloon(start, color, delay) {
      ...
      this.addActor((a, dt) => {
        ...
        if (g.position.y > 1880 || t > life) {
          ...
          for (let i = 0; i < 40; i++) {
            ...
            this.confetti.emit(g.position, d, i % 2 ? color : PARTY[i % PARTY.length], rand(10, 16), rand(2, 3.5), { gravity: 220, drag: 1.8, spin: 10, flutter: 120 });
          }
    ...
    goalFireworks(pos, team, n) {
      const palette = [team, WHITE, new THREE.Color(1, 0.8, 0.3), team.clone().offsetHSL(0.5, 0, 0), new THREE.Color().setHSL(Math.random(), 1, 0.6)];
      ...
      for (let r = 0; r < 8; r++) {
        ...
        this.addActor((a, dt) => {
          ...
          if (a.t - delay >= fuse) { this.fireworkBurst(p.clone(), color, 1); return false; }
          return true;
        });
      }
      this.addShake(1.2);
    ...
    fireworkBurst(p, color, generation) {
      ...
      for (let i = 0; i < (first ? 220 : 60); i++) {
        ...
        this.glow.emit(p.x, p.y, p.z, d.x, d.y, d.z, rand(1.1, 1.9), first ? rand(60, 100) : rand(30, 50), c, { drag: 1.5, gravity: 260, twinkle: first ? 0 : 1 });
      }
      ...
      for (let k = 0; k < 4; k++) {
        ...
        const wait = rand(0.3, 0.6), childColor = Math.random() < 0.5 ? WHITE : new THREE.Color(1, 0.8, 0.3);
        this.addActor(a => {
          if (a.t < wait) return true;
          this.fireworkBurst(child, childColor, 2);
          return false;
        });
      }
    }
    ...
    goalHellfire(pos, team, n) {
      const floor = new THREE.Vector3(pos.x * 0.5, 20, pos.z);
      this.burstFlash(pos, new THREE.Color(1, 0.55, 0.15), 2600, 0.5, 1);
      this.shockRing(new THREE.Vector3(floor.x, 6, floor.z), UP, new THREE.Color(1, 0.25, 0.05), 1800, 1.8, 0.35, 0.9);
      let carry = 0;
      this.addActor((a, dt) => {
        const k = a.t / 1.8;
        if (k >= 1) return false;
        ...
        for (; carry >= 1; carry--) {
          ...
          tmpC.setRGB(1, 0.25 + heat * 0.6, heat * 0.25);
          this.glow.emit(floor.x + Math.cos(ang) * r, floor.y, floor.z + Math.sin(ang) * r * 0.6,
      ...
      for (let i = 0; i < 300; i++) {
        tmpC.setRGB(1, rand(0.4, 0.7), rand(0.05, 0.2));
      ...
      this.lightFlash(pos, new THREE.Color(1, 0.45, 0.1), 6);
      this.addShake(2.6);
    }

    // Dueling Dragons: two dragons of fire spiral out of the goal around each other
    goalDragons(pos, team, n) {
      const colors = [team.clone().lerp(WHITE, 0.15), new THREE.Color(1, 0.62, 0.12)];
      const base = new THREE.Vector3(0, 80, pos.z);
      this.burstFlash(pos, colors[1], 1800, 0.4, 0.8);
      colors.forEach((color, d) => {
        const phase = d * Math.PI;
        const headMat = new THREE.SpriteMaterial({ map: GLOW_TEX, color, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false });
        ...
        this.addActor((a, dt) => {
          ...
          if (k >= 1) {
            ...
            for (let i = 0; i < 120; i++) {
              ...
              this.glow.emit(head.position.x, head.position.y, head.position.z, v.x, v.y, v.z, rand(0.6, 1.2), rand(60, 110), color, { drag: 1.8, gravity: 300 });
            }
            return false;
          }
          ...
          if (prev) {
            for (; carry >= 1; carry--) {
              ...
              this.glow.emit(q.x, q.y, q.z, rand(-60, 60), rand(-40, 80), rand(-60, 60), rand(0.35, 0.6), rand(150, 250), tmpC, { drag: 2.5, grow: -0.6, opacity: 0.85 });
              if (Math.random() < 0.25) this.glow.emit(q.x, q.y, q.z, rand(-200, 200), rand(-100, 200), rand(-200, 200), rand(0.8, 1.4), rand(25, 40), color, { drag: 1.5, gravity: 150, twinkle: 1 });
            }
          } else carry = 0;
          prev = p;
          return true;
    ...
    goalVoxel(pos, team, n) {
      ...
      const teamGlow = team.clone().multiplyScalar(1.8);
  ...
  return { Effects, SpeedShader, EXPLOSIONS };
})();
